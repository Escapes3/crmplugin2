...
class Contactpersoon {
    ...
    public function save($input_array){
        try {
            ...
            $wpdb->query($wpdb->prepare("INSERT INTO `". $wpdb->prefix
                ."crm_contactpersoon` ( `naam`, `email`, `telefoonnummer`)".
     ...
     */
    public function getNrOfContactpersoon(){
        global $wpdb;
     ...
     * @return type
     */
    public function getContactpersoonList(){

        global $wpdb;
        $return_array = array();

        $result_array = $wpdb->get_results( "SELECT * FROM `". $wpdb->prefix . "crm_contactpersoon` ORDER BY `id_contactpersoon`", ARRAY_A);
...
// New object
            $cat = new Contactpersoon();
            // Set all info
            $cat->setId($array['id_contactpersoon']);
    ...
    */
    private function getTableName(){
        ...
        global $wpdb;
        return $table = $wpdb->prefix . "crm_contactpersoon";
    }
    ...
    public function update($input_array){
        ...
        try {
            ...
            $table_fields = array( 'id_contactpersoon', 'naam' , 'email', 'telefoonnummer');
            $data_array = array();
            ...
            $wpdb->query($wpdb->prepare("UPDATE ".$this->getTableName(). " SET `naam` = '%s', `email` = '%s', `telefoonnummer` = '%s' ".
                "WHERE `wp_crm_contactpersoon`.`id_contactpersoon` =%d;",$input_array['naam'],
...
// therefore not be changed!
                if (!empty($table_data)){
                    unset($table_data['id_contactpersoon']);
                }
                break;
            // Remove
        }
        ...
        try {
           ...
           "` WHERE `id_event_category` = %d", $input_array['id']);
            ...
            $wpdb->delete( $this->getTableName(),
                array( 'id_contactpersoon' => $input_array['id'] ),
                array( '%d' ) ); // Where format
            //*/
